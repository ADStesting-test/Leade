...
class ScenarioElements:
    ...
    def single_ped_extraction(self, pedestrian_obj):
        ...
        if _ped_obj.has_pedestrian_type():
            ...
            _name = _ped_type.get_name()
            if _name is not None:
                single_pedestrian.name = _name
    ...
    def evaluate_test(self, test_case_dict):
        ...
        for k in range(len(test_case_dict['pedestrianList'])):
            ...
            if ped_k['destination'] is not None:
                ...
                new_test_case['pedestrianList'][k]['motion'].append(new_test_case['pedestrianList'][k]['destination'])
                new_test_case['pedestrianList'][k]['motion'][-1]['speed'] = new_test_case['pedestrianList'][k]['motion'][-2]['speed']
