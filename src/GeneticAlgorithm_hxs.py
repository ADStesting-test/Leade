...
class GAGeneration:
    ...
    def mutation(self, p, lane_config, crosswalk_config):
        ...
        for index_npc in range(no_npc):
            ...
            for wp in range(len(npc['motion'])):
                ...
                #####################
                mutated_horizontal_offset = gauss(npc_offset[1+wp], 1)
                print("tttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttttt",mutated_horizontal_offset)
                ####################
                if wp_lane == current_lane:
                    
                    ####################
                    # mutated_horizontal_offset = gauss(npc_offset[2+wp], 1)  # NPC相对于每个路点的水平方向偏移
                    mutated_horizontal_offset = float(np.clip(mutated_horizontal_offset, npc_offset_range[0], npc_offset_range[1]))
                    offset_element.list1d[_offset_count] = mutated_horizontal_offset
                    _offset_count += 1
                    ####################  
    ...
    def one_generation(self):
        ...
        for i in range(0, self.population_size - 1, 2):  # i = 0,2,4,...,n-3
            if i + 1 <= self.population_size - 2:
                ...
                print("new_population ========================================================= ",_new_population)
            else:
    ...
    def coverage_one_generation(self, coverage_population):
        ...
        lane_info = map_info.get_lane_config()
        crosswalk_info = map_info.get_crosswalk_config()
        assert len(coverage_population) == self.population_size
...
if __name__ == "__main__":
    ...
    results = ['result19-08-2021-10-05-47.json', 'result19-08-2021-10-05-47.json','result19-08-2021-10-05-47.json','result19-08-2021-10-05-47.json']
    population = []
